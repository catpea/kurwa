...
async function createNode(seed){
  ...
  await system.create($parent.id, JSON.stringify(node))
}
      ...
      <polyline fill="none" stroke="black" points="20,100 40,60 70,80 100,20" marker-mid="url(#circle)" />
        ...
        {#if $edges}
          {#each $edges as edge (edge.id)}
            <Anchor color={edge.color} node={$parent.id} edge={edge.id} x1={edge.output.left} y1={edge.output.top} x2={edge.input.left} y2={edge.input.top} {translate} on:disconnect={destroyEdge}/>
          {/each}
        {/if}
      </svg>
