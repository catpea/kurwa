...
export let location = writable(); // id of database node, must be owned by user
...
onMount(() => {
  unsubscribe.push(location.subscribe(async value=>{
    ...
    const nodes1 = await system.nodes($location);
    const edges1 = await system.edges($location);
